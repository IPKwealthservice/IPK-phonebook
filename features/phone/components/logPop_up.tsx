...
export default function CallFollowUpModal({
  ...
  const [clientCodeModalOpen, setClientCodeModalOpen] = useState(false);
  const datePickerMountedRef = useRef(false);
  const previousStageRef = useRef<ClientStageOption | null>(null);
  ...
  const handleClientCodeCancel = useCallback(() => {
    if (!clientCode.trim()) {
      setSelectedStage(previousStageRef.current);
    }
    setClientCodeModalOpen(false);
  }, [clientCode]);

  const handleClientCodeSave = useCallback(() => {
    if (!clientCode.trim()) {
      Alert.alert("Client Code Required", "Please enter a client code.");
      return;
    }
    setClientCodeModalOpen(false);
  }, [clientCode]);
  ...
  const handleClose = useCallback(() => {
    if (!allowDismiss || saving) return;
    setShowDatePicker(false);
    setStagePickerOpen(false);
    setStageFilterPickerOpen(false);
    setClientCodeModalOpen(false);
    onClose();
  }, [allowDismiss, onClose, saving]);
  ...
  return (
    <>
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={allowDismiss ? handleClose : () => {}}
      >
        <KeyboardAvoidingView
        ...
        >
          ...
          <View style={styles.card}>
            ...
            >
              ...
              {/* Client Code entry - prompted via popup when Account Opened is selected */}
              {selectedStage === "ACCOUNT_OPENED" && (
                <View style={styles.field}>
                  <Text style={styles.label}>Client Code *</Text>
                  <Pressable
                    onPress={() => setClientCodeModalOpen(true)}
                    style={styles.dropdownButton}
                  >
                    <Text style={styles.dropdownText}>
                      {clientCode.trim() ? clientCode.trim() : "Enter client code"}
                    </Text>
      ...
      >
        <View style={styles.modalOverlay}>
          <Pressable
            style={StyleSheet.absoluteFillObject}
            onPress={handleClientCodeCancel}
          />
          <View style={styles.clientCodeCard}>
            <Text style={styles.optionCardTitle}>Enter Client Code</Text>
            <TextInput
              value={clientCode}
            ...
            />
            <View style={styles.clientCodeActions}>
              <Pressable onPress={handleClientCodeCancel} style={styles.clientCodeBtn}>
                <Text style={styles.clientCodeBtnText}>Cancel</Text>
              </Pressable>
              <Pressable onPress={handleClientCodeSave} style={styles.clientCodeBtnPrimary}>
                <Text style={styles.clientCodeBtnTextPrimary}>Save</Text>
              </Pressable>
            </View>
          </View>
        </View>
            ...
            {CLIENT_STAGE_OPTIONS.map((option) => (
              <Pressable
                ...
                onPress={() => {
                  ...
                  setStagePickerOpen(false);
                  if (option === "ACCOUNT_OPENED") {
                    previousStageRef.current = selectedStage ?? null;
                    setClientCodeModalOpen(true);
                  }
                }}
